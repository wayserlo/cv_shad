...
def assert_dtypes_compatible(actual_dtype, correct_dtype):
    __tracebackhide__ = True
    assert np.can_cast(actual_dtype, correct_dtype, casting="same_kind") and np.can_cast(
        correct_dtype, actual_dtype, casting="same_kind"
    ), (
...
def assert_shapes_match(actual_shape, correct_shape):
    __tracebackhide__ = True
    assert len(actual_shape) == len(correct_shape) and actual_shape == correct_shape, (
...
def assert_ndarray_equal(*, actual, correct, rtol=0, atol=1e-6, err_msg=""):
    ...
    np.testing.assert_allclose(actual, correct, atol=atol, rtol=rtol, verbose=True, err_msg=err_msg)
